 ...
 */
export function extractFrontMatter(document: string): { title: string | null; [key: string]: JSONValue } {
    ...
    if (!result.title) {
        ...
        if (h1Match) {
            result.title = h1Match[1]?.trim() || null
        }
    }

    return { ...result, title: (result?.title as string | undefined) ?? null }
}
