...
describe('Query TurboPuffer Simple', () => {
    ...
    test('should insert and query chunks from TurboPuffer', async () => {
        ...
        // Handle TurboPuffer response structure
        const vectorResultsArray = Array.isArray(vectorResults)
            ? vectorResults
            : vectorResults.rows || vectorResults.data || []
    ...
    test('should handle topK parameter correctly', async () => {
        ...
        // Clean up
        await nukeTurbopufferNamespace({ organizationId, namespaceId: namespaceId + '_topk' })
    }, 30000)
})
