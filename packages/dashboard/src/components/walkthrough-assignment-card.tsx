...
import { GripVertical, ToggleLeft, ToggleRight, Trash2, UsersIcon, MapIcon } from 'lucide-react'
import { use, useEffect, useState } from 'react'
import { toast } from 'sonner'

import { assignWalkthroughsToServerAction, reorderServerWalkthroughsAction, updateWalkthroughAssignmentAction, removeWalkthroughAssignmentAction } from '@/lib/orpc/actions'
...
function SortableWalkthrough({ walkthrough, onToggle, onDelete }: { walkthrough: AssignedWalkthrough; onToggle: () => void; onDelete: () => void }) {
    const {
        attributes,
        listeners,
        setNodeRef,
...
export function WalkthroughAssignmentCard({ serverId, availableWalkthroughsPromise, assignedWalkthroughsPromise }: WalkthroughAssignmentCardProps) {
    ...
    useEffect(() => {
        if (selectedWalkthroughIds.length === assignedWalkthroughs.length &&
            selectedWalkthroughIds.every(id => assignedWalkthroughs.some(w => w.id === id))) {
            return
        }

        const newAssignments = selectedWalkthroughIds.map((id, index) => {
        ...
        <>
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Manage interactive walkthroughs for this server
                    </CardDescription>
                </CardHeader>
            ...
            <AlertDialog open={!!deleteWalkthroughId} onOpenChange={(open) => !open && setDeleteWalkthroughId(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will remove the walkthrough from this server. The walkthrough itself will not be deleted and can be reassigned later.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
